...
function convertToIST24HourFormat(utcDateStr: string): string {
  // Step 1: Parse input string into a Date object (assume UTC)
  const date = new Date(`2025 ${utcDateStr} UTC`); // Add year + UTC context

  if (isNaN(date.getTime())) return "Invalid date";

  // Step 2: Convert to IST by adding 5 hours 30 minutes
  const istDate = new Date(date.getTime() + (5.5 * 60 * 60 * 1000));

  // Step 3: Format as "DD MMM, HH:mm"
